...
function DoingBoard({ title }) {
  console.log(`DOING Rendered`);

  let { data } = useQuery({
    queryKey: ["doing"],
    queryFn: () => {
      return axios.get("https://kanban-board-server-eight.vercel.app/get-todos?collection=doing");
    },
  });
  ...
  let { mutateAsync } = useMutation({
    mutationFn: (newtodo) => {
      return axios.post(
        `https://kanban-board-server-eight.vercel.app/create-todos?collection=doing`,
        newtodo
      );
    },
    onSuccess: async () => {
      console.log(`Created todo`);
      await queryClient.invalidateQueries({ queryKey: ["doing"] });
      let { data } = queryClient.getQueryData({ queryKey: ["doing"] });
      console.log(data);
      setCards(data);
    },
  });
  ...
  let { mutateAsync: deleteMutateAsync } = useMutation({
    mutationFn: (id) => {
      return axios.delete(
        `https://kanban-board-server-eight.vercel.app/delete-todos/${id}?collection=doing`
      );
    },
    onSuccess: async () => {
  ...
  let modify = async (from, to, item) => {
    let res = await axios.post(
      `https://kanban-board-server-eight.vercel.app/sorting?collection=doing`,
      {
        from,
        to,
        item,
  ...
  let modifyAdd = async (from, to, item) => {
    let res = await axios.post(
      `https://kanban-board-server-eight.vercel.app/sorting-add?collection=doing`,
      {
        from,
        to,
        item,
        ...
        <article id="list" className="mx-2">
          <ReactSortable
            ...
            setList={(list) => {
              setCards(list);
              ulist = list;
              console.log("Doing ", ulist);
            }}
            list={cards}
            onSort={(e) => {
              console.log(`sort from doing`);
              console.log(e);
...
export default memo(DoingBoard);
