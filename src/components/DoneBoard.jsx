...
function DoneBoard({ title }) {
  console.log(`DONE Rendered`);

  let { data } = useQuery({
    queryKey: ["done"],
    queryFn: () => {
      return axios.get("https://kanban-board-server-one.vercel.app/get-todos?collection=done");
    },
  });
  ...
  let { mutateAsync } = useMutation({
    mutationFn: (newtodo) => {
      return axios.post(
        `https://kanban-board-server-one.vercel.app/create-todos?collection=done`,
        newtodo
      );
    },
    onSuccess: async () => {
      console.log(`Created todo`);
      await queryClient.invalidateQueries({ queryKey: ["done"] });
      let { data } = queryClient.getQueryData({ queryKey: ["done"] });
      console.log(data);
      setCards(data);
    },
  });
  ...
  let { mutateAsync: deleteMutateAsync } = useMutation({
    mutationFn: (id) => {
      return axios.delete(
        `https://kanban-board-server-one.vercel.app/delete-todos/${id}?collection=done`
      );
    },
    onSuccess: async () => {
  ...
  let modify = async (from, to, item) => {
    let res = await axios.post(
      `https://kanban-board-server-one.vercel.app/sorting?collection=done`,
      {
        from,
        to,
        item,
  ...
  let modifyAdd = async (from, to, item) => {
    let res = await axios.post(
      `https://kanban-board-server-one.vercel.app/sorting-add?collection=done`,
      {
        from,
        to,
        item,
        ...
        <article id="list" className="mx-2">
          <ReactSortable
            ...
            setList={(list) => {
              setCards(list);
              ulist = list;
              console.log("done ", ulist);
            }}
            list={cards}
            onSort={(e) => {
              console.log(`sort from done`);
              console.log(e);
